...
export default function InitialAlert() {
  ...
  return isClient ? (
    <AlertDialog open={shouldEmitAlert}>
      ...
      <AlertDialogContent className="max-h-[100vh] space-y-0 p-4 text-center max-[300px]:overflow-y-scroll sm:min-h-[50vh] sm:min-w-[35vw] sm:p-8">
        <AlertDialogHeader>
          <AlertDialogTitle className="mb-2 text-center text-3xl tracking-widest sm:mb-6 sm:text-4xl">
            {t.rich('title', {
              span: (chunks) => (
        ...
        <AlertDialogFooter className="items-end max-sm:space-y-3 sm:gap-3">
          <div className="w-full max-sm:mr-auto max-sm:mt-2">
            <div className="my-2 flex items-center space-x-2 sm:hidden">
              <Checkbox
                checked={checked}
                onCheckedChange={(value) => setChecked(Boolean(value))}
                id="terms"
